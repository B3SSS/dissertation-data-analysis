...
cursor.execute("""
    SELECT dci.cityName, COUNT(*)
    FROM Dim_City AS dci
        JOIN Dim_District AS ddis USING (cityid)
        JOIN Dim_Citizen AS dc USING (districtId)
...
clickhouse_client.insert("aggregation_city", data=result, column_names=["city", "quantity"])
